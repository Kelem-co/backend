...
from kelem_co_backend.users.api.schema import UpdateUserSchema
from kelem_co_backend.users.api.schema import UserSchema
...
def update_current_user(request, data: UpdateUserSchema):
    user = request.user
    user.name = data.name  # type: ignore[attr-defined]
    user.username = data.username  # type: ignore[attr-defined]
    user.save()
    return user
