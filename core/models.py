...
class ImportJob(TimeStampedModel, UUIDModel):
    ...
    task_id = models.CharField(_("Celery Task ID"), max_length=255, null=True, blank=True)
    file = models.FileField(_("Uploaded File"), upload_to="imports/")
    module = models.CharField(_("Module"), max_length=50) # 'students', 'parents', 'teachers'
    organization = models.ForeignKey("organizations.Organization", on_delete=models.CASCADE, related_name="import_jobs")
    branch = models.ForeignKey("branches.Branch", on_delete=models.CASCADE, related_name="import_jobs")
    errors = models.JSONField(_("Errors"), null=True, blank=True)
    progress = models.IntegerField(_("Progress"), default=0)
    created_by = models.ForeignKey("accounts.User", on_delete=models.SET_NULL, null=True, related_name="import_jobs")
