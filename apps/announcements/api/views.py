...
class AnnouncementViewSet(viewsets.ModelViewSet):
    ...
    """
    queryset = Announcement.objects.prefetch_related('targeted_grades', 'targeted_sections').all()
    ...
    def get_queryset(self):
        ...
        if is_urgent is not None:
            qs = qs.filter(is_urgent=is_urgent.lower() in ['true', '1', 't', 'y', 'yes'])
    ...
    def get_targeting_criteria(self, request):
        ...
        sections = scope_queryset_for_user(Section.objects.select_related("grade").all(), user)
        
        # Serialize the data simply to return as JSON
        grade_data = [{"id": str(g.id), "name": g.name, "level": g.level} for g in grades]
        section_data = [{"id": str(s.id), "name": s.name, "grade_name": s.grade.name if s.grade else None} for s in sections]
